...
modelRun <- function(
    ...
    if(space_only){
        ...
        Params$z <- array(0,dim=c(field$mesh$n, 1))
    }
...
modelList$notime$mixture <- modelRun(
    pointDF, polyDF = polyDF, nugget = FALSE, time_structured=TRUE, space_only=TRUE,
    time_unstructured = FALSE, survey_effect = TRUE, priors = 1)

modelList$notime$point <- modelRun(
    pointDF, polyDF = NULL, nugget = FALSE, time_structured = TRUE, space_only=TRUE,
    time_unstructured = FALSE, survey_effect = FALSE, priors = 1)

modelList$notime$resample <- modelRun(
    bind_rows(pointDF, ihmePolyDF), nugget = FALSE, time_structured=TRUE, space_only=TRUE,
    time_unstructured = FALSE, survey_effect = TRUE, priors = 1)
...
# 
predList <- list()
predList$notime <- lapply(modelList$notime, simulateDataFieldCI, field=field)
...
saveRDS(
    ...
    sprintf(
        "~/Data/dataRun/no_time_model_y%s_r%s_.RDS",
        yearHO,
        regHO))
